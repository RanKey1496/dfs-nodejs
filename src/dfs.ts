class Graph {
    ...
    private nodes: Map<number, Array<{key: number, x: number, y: number}>>;

    constructor() {
        this.nodes = new Map();
    }

    public addVertex(v: {key: number, x: number, y: number}) {
    ...
    public addEdge(v: {key: number, x: number, y: number}, w: {key: number, x: number, y: number}) {        
        this.nodes.get(v.key).push({key: w.key, x: w.x, y: w.y});
        this.nodes.get(w.key).push({key: v.key, x: v.x, y: v.y});
    }

    public dfs(start: {key: number, x: number, y: number}) {
        const visited = [];
    ...
    private DFSUtil(verticle: {key: number, x: number, y: number}, visited: Array<boolean>) {
